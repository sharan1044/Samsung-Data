...
names(label)[1] <- "Label"
my_data <- cbind(label, my_data)

## Defining the result data frame to be written onto the final txt file
result_data <- data.frame(matrix(nrow=0, ncol = 87))

## Adding the averages of all variables for all activities to the result dataset
y <- split(my_data, my_data$Label)
for(i in 1:nrow(activities)){
  x <- sapply(y[[i]], mean)
  result_data <- rbind(result_data, x)
}
names(result_data) <- names(my_data)
result_data[,1] <- sort(activities[,2])

## Adding the averages of all variables for all subjects to the result dataset
my_data[,1] <- subject[,1]
names(my_data)[1] <- "Subject"

y <- split(my_data, my_data$Subject)
for(i in 1:30){
  x <- sapply(y[[i]], mean)
  result_data <- rbind(result_data, x)
}
...
names(result_data)[1] <- "Activities/Subject"

# Writing the result data set ontoa txt file
write.table(result_data, file = "./result_data.txt", row.names = FALSE)
